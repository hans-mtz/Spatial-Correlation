...
In the DGP, $e_l\sim\mathcal{G}_{exp}(c_{\bar\rho})$ and $X\sim[1 \;\mathcal{G}_{exp}(c_{\bar\rho}) \;\mathcal{G}_{exp}(c_{\bar\rho})]$. The average pairwise correlation of the 250 observations is $\bar\rho=0.03$. 
...
I used 1,000 iterations to calculate the rejection frequencies and the average AR(1) coefficients of the residuals.

@tbl-kernel displays the rejection frequencies for the different estimators. 

@tbl-ar1-mean displays the average AR(1) coefficients of the residuals.
...
kernel_results <- read.csv("../Simulations/Products/crank_bs_ear1_sims_res.csv")
...
e_ar1_tbl <- read.csv("../Simulations/Products/crank_bs_ear1_sims_ar1.csv")
