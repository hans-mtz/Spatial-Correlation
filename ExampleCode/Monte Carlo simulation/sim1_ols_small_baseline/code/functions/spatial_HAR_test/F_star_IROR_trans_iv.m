...
function [IROR_trans] = F_star_IROR_trans_iv(R,r,ydep,x_reg,z_mat,loc,K1,K2)
    ...
    theta_hat = (z_mat'*x_reg)\(z_mat'*ydep);
    
    G = z_mat'*x_reg/N;
    W = (z_mat'*z_mat/N)\eye(p,p);
    H = G'*W*G;
    iH = H\eye(p,p);

    e_hat= ydep-x_reg*theta_hat ;
    xe_hat_all = repmat(e_hat,1,p).*z_mat*W'*G;
