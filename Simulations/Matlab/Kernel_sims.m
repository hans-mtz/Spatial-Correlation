...
rej_freq = zeros(1,N_models);
for m=1:N_models
    rej_freq_m = 0;
    for r=1:N_reps
        % generate data      
        [y, X, D_mat] = DGP(beta,T,k,s,rho_bar,m);
%         writetable(table(y, X, s), strcat('../Stata/data_',num2str(m),'.csv')); % Saving
        ...
        rej_freq_m = rej_freq_m + mean(rej_i)/N_reps;      
    end
    rej_freq(m) = rej_freq_m;
end

fprintf('Rejection frequency was iid spatial (%5.4g, %5.4g)\n',rej_freq);
