...
% Model 2 Spatial - Demeaned outcome
% e~G_exp(c_rho_bar) 
% X~[1 G_exp(c_rho_bar) G_exp(c_rho_bar)]
...
rej_freq_m_i = NaN(N_reps,k,N_estimators, N_models);
...
%         writetable(table(y, X, s), strcat('../Stata/data_splines.csv')); % Saving
    % for m=1:2
        ...
        e_ar1_betas(r,:,1,m) = ols(u_t, u_t_1, u_t_1, cholesky_flag);
        ...
        rej_freq_m_i(r,:,1,m) = rej_hr;
        
        t_stat_array(r,:,1,m) = t_stat_hr;
        num_array(r,:,1,m) = H_0;
        den_array(r,:,1,m) = se_hr;

        t_stat_scpc = H_0./se_scpc;
        rej_scpc = abs(t_stat_scpc) > abs(cv_scpc);
        rej_freq_m_i(r,:,2,m) = rej_scpc;
        
        t_stat_array(r,:,2,m) = t_stat_scpc;
        num_array(r,:,2,m) = H_0;
        den_array(r,:,2,m) = se_scpc;
        ...
        for l=L
            ...
            rej_freq_m_i(r,:,i,m) = rej_kernel;
            
            t_stat_array(r,:,i,m) = t_stat_kernel;
            num_array(r,:,i,m) = H_0;
            den_array(r,:,i,m) = se_kernel;
            i=i+1;
        end
        % Kernel + BSplines
        j=i; 
...
%                 X_bs = [X S];
                ...
                e_ar1_betas(r,:,n,m) = ols(u_t_bs, u_t_1_bs, u_t_1_bs, cholesky_flag);
...
%                     disp(q);
                    ...
                    rej_freq_m_i(r,:,j,m) = [NaN rej_kernel_bs];
                    
                    
                    t_stat_array(r,:,j,m) = [NaN t_stat_kernel_bs];
                    num_array(r,:,j,m) = [NaN H_0_bs];
                    den_array(r,:,j,m) = [NaN se_kernel_bs(1:k-1)];
